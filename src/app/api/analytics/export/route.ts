...
import { generateAnalyticsChartPdf } from "@/lib/analytics/chartPdf";
...
export async function GET(request: NextRequest) {
  let user;
  try {
    user = await requirePermission("analytics", "view");
  } catch (err) {
  ...
  if (format === "pdf") {
    ...
    const buffer = await generateAnalyticsChartPdf({
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: { "Content-Type": "application/pdf", "Content-Disposition": "attachment; filename=analytics-performance.pdf" },
    });
  }

  const rows = scoresToExportRows(scores);
  const buffer = await rowsToExcelBuffer(rows, "Analytics Performance");
  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      "Content-Disposition": "attachment; filename=analytics-performance.xlsx",
    },
  });
}
